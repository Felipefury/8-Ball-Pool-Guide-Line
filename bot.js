...
var { overlayWindow } = require("electron-overlay-window")
...
init = function() {
   ...
   function createWindow() {
      ...
      win = new BrowserWindow({
         ...
         ...(settings.overlay) && {...overlayWindow.WINDOW_OPTS}
      });
      ...
      if(settings.overlay) {
         ...
         overlayWindow.attachTo(win, settings.sourceName)

         overlayWindow.on('moveresize', ev => {
            settings.windowLocation = ev
         });

         overlayWindow.on('attach', ev => {
            setTimeout(() => {
               setInterval(() => {
                  win.webContents.send('moveresize', settings.windowLocation);
               }, 30);
               win.webContents.send('attach', ev, settings);
            }, 4000);
         });
   ...
   register = function() {
      globalShortcut.register('Z', () => {
         ...
         if(!settings.overlay) {
            win.setPosition(screen.getCursorScreenPoint().x, screen.getCursorScreenPoint().y)
         };
         win.webContents.send('key', 'Z', screen.getCursorScreenPoint());
      });
      ...
      if ("bot" == settings.location) {
         ...
         globalShortcut.register('K', () => { win.webContents.send('key', 'K') });
      ...
      if ("string" == typeof arg) {
         ...
         switch (arg.info) {
            case "resize":
               ...
               break;
            case "location":
               settings.location = arg.location;
            ...
            case "source":
               ...
               settings.sourceName = arg.data[0]
               settings.overlay = true
               settings.id = arg.data[1]
