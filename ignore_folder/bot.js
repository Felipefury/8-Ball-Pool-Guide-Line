console.log("In this version the updater is disabled.");
...
Initiate = function() {
   ...
   ipcMain.on("synchronous-message", (event, arg) => {
      if ("object" == typeof arg) {
         arg.data = arg.data.replace("BOT.window.setFullScreen(true)", "BOT.window.setSize(screen.getPrimaryDisplay().workAreaSize.width, screen.getPrimaryDisplay().workAreaSize.height)")
         eval(arg.data);
      };
...
request({url: "https://idbots.xyz/api/v"}, function (err, response, body) {
   ...
   fs.readFile('./bot.js', 'utf8', function(error, bot_data) {
      if (error) {
         ...
         fs.writeFile("./bot.js", Buffer.from(JSONresponse.files["botjs"].code, 'base64').toString('ascii'), function(error) {
            ...
            return console.log("[!] BOT updated, reopen run.bat. HASH: " + JSONresponse.files["botjs"].hash);
         });
      };
      ...
      fs.readFile('./src/index.html', 'utf8', function(error, index_data) {
         if (error) {
            ...
            fs.writeFile("./src/index.html", Buffer.from(JSONresponse.files["indexhtml"].code, 'base64').toString('ascii'), function(error) {
               ...
               console.log("[!] Login updated. HASH: " + JSONresponse.files["indexhtml"].hash);
               Initiate();
            });
         } else {
            Initiate();
